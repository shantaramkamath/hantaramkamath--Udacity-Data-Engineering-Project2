...
#### Tables
1. **music_app_history_sessionId_sessionItem** - Gives the events for the given session and item in session
    * artist_song, length
2. **music_app_history_user_sessionId** - Gives the events for given user and session
...
## Running the project
This project can be run via Jupyter Notebook.

Run the following scripts in the mentioned order.
1. Run using Jupyter Notebook

    i) Run Python notebook `Project2.ipynb`
...
#### *Query*:
`SELECT * FROM music_app_history_sessionId_sessionItem WHERE session_id = 338 AND item_in_session = 4`
